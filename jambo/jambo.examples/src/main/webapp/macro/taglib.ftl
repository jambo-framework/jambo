...
<#include "base/input.ftl"/>
<#include "base/select.ftl"/>
<#include "base/checkboxlist.ftl"/>
<#include "base/textarea.ftl"/>
<#include "base/file.ftl"/>
<#include "base/button.ftl"/>
<#include "base/date.ftl"/>
<#include "base/editor.ftl"/>
<#include "base/time.ftl"/>
<#include "base/tree.ftl"/>
<#include "base/table.ftl"/>
<#include "base/form.ftl"/>
<#include "base/modal.ftl"/>
<#include "code2name.ftl"/>
...
<#--图片通用弹窗-->
<#--modalName:弹出窗的名称-->
<#--调用方法open弹出窗口名称，例如:窗口名称 abc ，调用openImage('标题','图片地址')-->
<#macro imageDialog modalName>
	<div class="modal fade" id="${modalName}ImageDialog" style="overflow-y: hidden;">
	  <div class="modal-dialog" style="width: 80%">
	    <div class="modal-content">
	      ...
	      <!--startprint-->
	      <div class="modal-body" id="${modalName}Body">
              <img id="${modalName}Image" src="" class="img-thumbnail">
	      </div>
	    </div><!-- /.modal-content -->
	  </div><!-- /.modal-dialog -->
	</div><!-- /.modal -->
	<script>
		function open${modalName}(image) {
			try {
				if (image) {
                    $("#${modalName}Image").attr("src", image);
				}
			} catch(e){
				alert("图片找不到");
			}
			$('#${modalName}ImageDialog').modal();
		}
		 function close${modalName}() {
				$('#${modalName}ImageDialog').modal('hide');
			}
	</script>
</#macro>
...
<#macro uploadImg path inputName size filetype msg imgs maxSize>
					...
					<#if img!="">
						<li id="SWFUpload_9_${img_index}">
							<div class="displayimg">
								<img src="${basePath}/${img}" onclick="openPreviewModel('${img}')">
							</div>
	...
	<script type="text/javascript">
		    ...
		    /**
		    *提供给弹窗显示图片使用
			 * 2015.12.19 jinbo 这个方法应该是没用到，而且跟注释说明不一致
		    */
...
<#--maxSize:默认文件大小 单位m-->
<#macro uploadFile path inputName size filetype msg  maxSize callBack isRename value="">
	<div id="ms__uploadPic_${inputName}">
		<div class="divUploader"><input type="button" id="up_file_${inputName}" />
		<#if msg!="">
		<div class="alert alert-warning alert-dismissable">${msg}</div>
		...
		<input type="hidden" size="100" name="${inputName}" id="${inputName}" value="${value?default('')}" />
	</div>
	<script type="text/javascript">
		    $(function ()  {
		        $('#ms__uploadPic_${inputName}').swfupload({
...
<#--isRename:是否重命名true:将使用时间命名，否则使用上传是文件名，可能会出现同名称覆盖的问题,中文也会存在问题-->
<#--maxSize:默认文件大小 单位m-->
<#macro uploadFile2 path inputName size filetype msg  maxSize callBack isRename value="">
	<div id="ms__uploadFile2_${inputName}">
		<div class="divUploader"><input type="button" id="up_File2_${inputName}" />
		<#if msg!="">
		<div class="alert alert-warning alert-dismissable">${msg}</div>
		</#if>
		</div>
		<input type="text" class="form-control" readonly="true" name="${inputName}" id="${inputName}" value="${value?default('')}" style="width:80%"/>
		<span class="front-cover" onclick="removeVideo${inputName}(this);">
			<a class="btn btn-xs red tooltips " data-original-title="点击删除文件" style="border-top-width: 10px;"><i class="glyphicon glyphicon-trash"></i></a>
		</span>
	</div>
	<script type="text/javascript">
		    $(function ()  {
		        $('#ms__uploadFile2_${inputName}').swfupload({
		            upload_url: "${basePath}/upload",
		            post_params:{"uploadPath":"/${path}","isRename":"${isRename?default("true")}","maxSize":"${maxSize}","allowedFile":"${filetype}","allowedFile":""},
		            file_size_limit: ${maxSize?default("1")}*1024,
		            file_types: "${filetype?default("*.jpg;*.png;*.gif;*.bmp;*.jpeg")}",
		            file_types_description: "Media files",
		            file_upload_limit: ${size?default("1")},
		            flash_url: "${basePath}/jquery/swfupload/swfupload.swf",
		            button_image_url: "${basePath}/jquery/swfupload/button_file.png",
		         	button_width: 92,
					button_height: 34,
		            button_placeholder: $('#up_File2_${inputName}')[0],
		            debug: false
		        })
				.bind('fileQueued', function (event, file) {
				   if (($("#ms__uploadFile2_${inputName} .msUploadImgs").children().length>=${size?default("5")}) || ($("#${inputName}").val() != "")) {
						  $(this).swfupload('cancelUpload');
						  alert("最多上传${size}个文件!");
					} else {
					    $(this).swfupload('startUpload');
				...
				.bind('uploadStart', function (event, file) {
						$('#ms__uploadFile2_${inputName} ul li#' + file.id).find('span.front-cover').text('0%');
				})
				.bind('uploadProgress', function (event, file, bytesLoaded) {
					   var percentage = Math.round((bytesLoaded / file.size) * 100);
				})
            ...
            function removeVideo${inputName}(uid) {
                var swfu = $.swfupload.getInstance('#ms__uploadFile2_${inputName}');
                var stats = swfu.getStats();
                stats.successful_uploads--;
                swfu.setStats(stats);
                $("#${inputName}").val("");
